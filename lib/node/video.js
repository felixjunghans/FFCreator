...
class FFVideo extends FFImage {
  constructor(conf) {
    ...
    this.voImageExtra = conf.voImageExtra === undefined ? 'png' : conf.voImageExtra;
  ...
  extractVideo() {
    return new Promise((resolve, reject) => {
      ...
      let opts = `-loglevel info -pix_fmt rgba -start_number 0 -vf fps=${fps},format=rgba -qscale:v ${qscale}`.split(
        ' ',
      );
